...
export function ServiceCard({ service }: ServiceCardProps) {
  ...
  return (
    <Card className="flex flex-col overflow-hidden rounded-lg shadow-md transition-shadow duration-300 hover:shadow-xl group relative focus-within:ring-2 focus-within:ring-white">
      ...
      <CardHeader>
        <CardTitle className="font-headline text-xl leading-tight transition-colors">
          <Link href={`/service/${service.id}`} className="hover:text-primary focus:outline-none after:absolute after:inset-0">
            {service.name}
          </Link>
        </CardTitle>
      </CardHeader>
      <CardContent className="flex-grow flex flex-col">
        ...
        <div className="flex justify-between items-center mt-auto pt-4 border-t border-border relative z-10">
            <p className="text-lg font-semibold text-primary dark:text-accent font-mono">
                {getFormattedPrice(service.price)}
                {service.unit === 'hr' && <span className="text-sm font-normal text-neutral-700 dark:text-neutral-300"> / hr</span>}
            </p>
